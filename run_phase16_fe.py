...
new_imp2.to_csv('output/phase16_feature_importance.csv', index=False)
...
sub.to_csv('output/submission_phase16.csv', index=False)
shutil.copy('output/submission_phase16.csv', os.path.join(DRIVE_CKPT_DIR, 'submission_phase16.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"  submission_phase16.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 16 Results ===", flush=True)
print("=" * 60, flush=True)
...
print(f"Phase 16:   {ensemble_cv:.4f}", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    ax.set_title('Feature Importance Top 30 (Phase 16)')
    ...
    plt.tight_layout(); plt.savefig('output/feature_importance_phase16.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase16.png saved", flush=True)

print("\n=== Phase 16 Complete ===", flush=True)
