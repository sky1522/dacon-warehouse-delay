...
K = int(os.environ.get('TOPK_K', '200'))
PIPELINE_VERSION = f"phase20_topk{K}_v1"
...
print(f"\n=== Phase 17 total: {len(final_feature_cols)} features ===", flush=True)

new_imp.to_csv('output/phase19_feature_importance_p17.csv', index=False)
del X_tmp, lgb_selector; gc.collect()


# ##############################################################
# Part 2B: Phase II.B 30 Strong Features
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Part 2B: Phase II.B 30 Strong Features ===", flush=True)
print("=" * 60, flush=True)
...
# Category A: Tier 1 Derived (5)
combined['robot_idle_ratio'] = (combined['robot_idle'] / (combined['robot_total'] + 1)).astype('float32')
combined['inflow_per_pack'] = (combined['order_inflow_15m'] / (combined['pack_station_count'] + 1)).astype('float32')
combined['battery_health'] = (combined['battery_mean'] * (1 - combined['low_battery_ratio'])).astype('float32')
combined['charging_demand_ratio'] = (combined['robot_charging'] / (combined['robot_total'] + 1)).astype('float32')
combined['inflow_normalized'] = (combined['order_inflow_15m'] / (combined['robot_total'] + combined['pack_station_count'] + 1)).astype('float32')

# Category B: Pack_station_count Normalization (5)
combined['pack_per_congestion'] = (combined['pack_station_count'] / (combined['congestion_score'] + 1)).astype('float32')
combined['pack_per_zone_density'] = (combined['pack_station_count'] / (combined['max_zone_density'] + eps_2b)).astype('float32')
combined['sku_per_pack'] = (combined['sku_concentration'] / (combined['pack_station_count'] + 1)).astype('float32')
combined['inflow_per_robot'] = (combined['order_inflow_15m'] / (combined['robot_total'] + 1)).astype('float32')
combined['inflow_per_active_robot'] = (combined['order_inflow_15m'] / (combined['robot_active'] + 1)).astype('float32')

# Category C: Efficiency / Congestion (4)
combined['forecast_acc_per_congestion'] = (combined['daily_forecast_accuracy'] / (combined['congestion_score'] + 1)).astype('float32')
combined['kpi_per_congestion'] = (combined['kpi_otd_pct'] / (combined['congestion_score'] + 1)).astype('float32')
combined['charge_eff_per_congestion'] = (combined['charge_efficiency_pct'] / (combined['congestion_score'] + 1)).astype('float32')
combined['maintenance_per_congestion'] = (combined['maintenance_schedule_score'] / (combined['congestion_score'] + 1)).astype('float32')

# Category D: Battery Compound (2)
combined['low_bat_x_battery_mean'] = (combined['low_battery_ratio'] * combined['battery_mean']).astype('float32')
combined['battery_mean_per_std'] = (combined['battery_mean'] / (combined['battery_std'] + 1)).astype('float32')

# Category E: 3-way Queueing + Service Impediment (2)
combined['queueing_3way'] = (combined['order_inflow_15m'] / (
    (combined['pack_station_count'] + 1) * (1 + combined['congestion_score']/100 + eps_2b)
)).astype('float32')
cong_norm = (combined['congestion_score'] / 100).fillna(0).clip(0, 1)
cq_norm = (combined['charge_queue_length'] / 30).fillna(0).clip(0, 1)
blk_norm = (combined['blocked_path_15m'] / 14).fillna(0).clip(0, 1)
col_norm = (combined['near_collision_15m'] / 9).fillna(0).clip(0, 1)
combined['service_impediment'] = ((cong_norm + cq_norm + blk_norm + col_norm) / 4).astype('float32')
del cong_norm, cq_norm, blk_norm, col_norm

# Category F: Log / Sqrt Transformations (6)
combined['log_robot_idle'] = np.log1p(combined['robot_idle'].clip(lower=0)).astype('float32')
combined['log_robot_charging'] = np.log1p(combined['robot_charging'].clip(lower=0)).astype('float32')
combined['log_congestion'] = np.log1p(combined['congestion_score'].clip(lower=0)).astype('float32')
combined['log_max_zone_density'] = np.log1p(combined['max_zone_density'].clip(lower=0)).astype('float32')
combined['log_order_inflow'] = np.log1p(combined['order_inflow_15m'].clip(lower=0)).astype('float32')
combined['sqrt_low_battery'] = np.sqrt(combined['low_battery_ratio'].clip(lower=0)).astype('float32')

# Category G: Robot Capacity Indicators (3)
combined['robot_active_ratio'] = (combined['robot_active'] / (combined['robot_total'] + 1)).astype('float32')
combined['robot_deficit'] = (1 - combined['robot_active_ratio']).astype('float32')
combined['heavy_load_score'] = (
    combined['urgent_order_ratio'].fillna(0) +
    combined['heavy_item_ratio'].fillna(0) +
    combined['cold_chain_ratio'].fillna(0)
).astype('float32')

# Category H: Composite Stress (3)
combined['pack_severity'] = (combined['pack_utilization'] / ((combined['pack_station_count'] + 1) / 10)).astype('float32')
combined['pack_deficit'] = ((1 - combined['pack_utilization']) * combined['pack_station_count']).astype('float32')
combined['operational_stress'] = (
    combined['pack_utilization'].fillna(0) * 0.3 +
    combined['robot_active_ratio'].fillna(0) * 0.3 +
    combined['service_impediment'].fillna(0) * 0.4
).astype('float32')

PHASE2B_FEATURES = [
    'robot_idle_ratio', 'inflow_per_pack', 'battery_health',
    'charging_demand_ratio', 'inflow_normalized',
    'pack_per_congestion', 'pack_per_zone_density', 'sku_per_pack',
    'inflow_per_robot', 'inflow_per_active_robot',
    'forecast_acc_per_congestion', 'kpi_per_congestion',
    'charge_eff_per_congestion', 'maintenance_per_congestion',
    'low_bat_x_battery_mean', 'battery_mean_per_std',
    'queueing_3way', 'service_impediment',
    'log_robot_idle', 'log_robot_charging', 'log_congestion',
    'log_max_zone_density', 'log_order_inflow', 'sqrt_low_battery',
    'robot_active_ratio', 'robot_deficit', 'heavy_load_score',
    'pack_severity', 'pack_deficit', 'operational_stress',
]
assert len(PHASE2B_FEATURES) == 30

# NaN cleanup for Phase 2B features
for col in PHASE2B_FEATURES:
    combined[col] = np.nan_to_num(
        combined[col].values.astype('float32'),
        nan=0.0, posinf=1000.0, neginf=-1000.0
    )

existing_set = set(final_feature_cols)
new_2b = [f for f in PHASE2B_FEATURES if f not in existing_set]
final_feature_cols = final_feature_cols + new_2b
print(f"Phase II.B features: {len(PHASE2B_FEATURES)} total, {len(PHASE2B_FEATURES) - len(new_2b)} already in base, {len(new_2b)} new added", flush=True)
print(f"\n=== Final total: {len(final_feature_cols)} features ===", flush=True)


# ##############################################################
# Part 4B: Top-K Importance Selection (H6)
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Part 4B: Top-K Importance Selection ===", flush=True)
print("=" * 60, flush=True)

imp_path = 'output/phase19_feature_importance.csv'
if not os.path.exists(imp_path):
    raise FileNotFoundError(f"Need {imp_path} - run Phase 19 importance extraction first")

df_imp = pd.read_csv(imp_path)
print(f"Loaded importance ranking: {len(df_imp)} features", flush=True)

print(f"\n*** Selecting Top-{K} features ***", flush=True)

topk_features = df_imp.head(K)['feature'].tolist()

missing = set(topk_features) - set(final_feature_cols)
if missing:
    print(f"WARNING: {len(missing)} features in importance not in feature_cols", flush=True)
    print(f"First 5 missing: {list(missing)[:5]}", flush=True)
    topk_features = [f for f in topk_features if f in final_feature_cols]

original_n = len(final_feature_cols)
final_feature_cols = topk_features
print(f"Reduced: {original_n} -> {len(final_feature_cols)} features", flush=True)

df_topk = df_imp[df_imp['feature'].isin(topk_features)]
n_p2b = int(df_topk['is_p2b'].sum()) if 'is_p2b' in df_topk.columns else 0
n_p17 = int(df_topk['is_p17new'].sum()) if 'is_p17new' in df_topk.columns else 0
print(f"Top-{K} composition:", flush=True)
print(f"  Phase 2B features: {n_p2b}/{len(final_feature_cols)}", flush=True)
print(f"  Phase 17 NEW: {n_p17}/{len(final_feature_cols)}", flush=True)
print(f"  Other (base/lag/rolling/scenario_agg): {len(final_feature_cols) - n_p2b - n_p17}/{len(final_feature_cols)}", flush=True)
print(f"  Importance range: {df_topk['importance'].min():.0f} ~ {df_topk['importance'].max():.0f}", flush=True)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = f'output/ckpt_phase20_topk{K}_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
try:
    ...
    ckpt_path = f'output/ckpt_phase20_topk{K}_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
print(f"\n=== Phase 20 Top-{K} Ensemble CV ===", flush=True)
print(f"CV MAE: {ensemble_cv:.4f}", flush=True)
print(f"Phase 17 baseline: 8.4251", flush=True)
print(f"Phase 19 baseline: 8.4357", flush=True)
print(f"Improvement vs P19: {8.4357 - ensemble_cv:+.4f}", flush=True)
...
sub.to_csv(f'output/submission_phase20_topk{K}.csv', index=False)
shutil.copy(f'output/submission_phase20_topk{K}.csv',
            os.path.join(DRIVE_CKPT_DIR, f'submission_phase20_topk{K}.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"\nsubmission_phase20_topk{K}.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print(f"=== Phase 20 Top-{K} Results ===", flush=True)
print("=" * 60, flush=True)
print(f"Pre-selection:   {original_n}", flush=True)
print(f"Top-K selected:  {len(final_feature_cols)}", flush=True)
for n in model_names_available:
...
print(f"Phase 17:   8.4251", flush=True)
print(f"Phase 19:   8.4357", flush=True)
print(f"Phase 20 (K={K}): {ensemble_cv:.4f}", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    ax.set_title(f'Feature Importance Top 30 (Phase 20 K={K})')
    plt.tight_layout(); plt.savefig(f'output/feature_importance_phase20_topk{K}.png', dpi=150, bbox_inches='tight')
    print(f"  feature_importance_phase20_topk{K}.png saved", flush=True)

print(f"\n=== Phase 20 Top-{K} Complete ===", flush=True)
