...
# ============================================================
# Google Drive checkpoint helpers (from Phase 11b)
# ============================================================
...
# ============================================================
# 2. NEW: Queueing Theory Features (42)
# ============================================================
print("\n" + "=" * 60, flush=True)
...
for name in ['robot', 'charger', 'pack', 'loading']:
    ...
    combined[f'q_rho_{name}_inv'] = (1.0 / (1.0 - rho + EPS)).astype(np.float32)  # 1/(1-rho)
    combined[f'q_pk_{name}'] = (rho**2 / (1.0 - rho + EPS)).astype(np.float32)    # rho^2/(1-rho) P-K
...
print(f"\n  Phase 10 base features: ~319")
print(f"  Queueing theory NEW: {len(q_cols)}")
print(f"  Total: ~{319 + len(q_cols)}", flush=True)
...
# ============================================================
# 4. Tree 6 Models (Phase 10 identical)
# ============================================================
print("\n" + "=" * 60, flush=True)
...
ckpt_path = 'output/ckpt_phase12a_lgb_raw.pkl'
...
ckpt_path = 'output/ckpt_phase12a_lgb_huber.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_lgb_sqrt.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_xgb.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_cat_log1p.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_cat_raw.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_mlp.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
ckpt_path = 'output/ckpt_phase12a_tabnet.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
...
# ============================================================
# 7. Level 1 Weighted Average Ensemble
# ============================================================
...
# ============================================================
# 8. Level 2 LGB Stacking (8 models)
# ============================================================
...
# ============================================================
print("\n" + "=" * 60, flush=True)
print("=== Phase 12a Results ===", flush=True)
print("=" * 60, flush=True)
q_feature_count = len([c for c in feature_cols if c.startswith('q_')])
print(f"New features: Queueing Theory {q_feature_count}", flush=True)
print(f"Total features: {len(feature_cols)}", flush=True)
...
print(f"Level 1 Tree+NN (8 models):  {all_cv:.4f}", flush=True)
print(f"Level 2 LGB Stacking:        {meta_lgb_cv:.4f}", flush=True)
print(f"Final selection:              {best_name} ({final_cv:.4f})", flush=True)
print(f"Phase 10 baseline:            8.577", flush=True)
print(f"Phase 10 vs Phase 12a:        {final_cv - 8.577:+.4f}", flush=True)
...
submission.to_csv('output/submission_phase12a.csv', index=False)

# Drive copy
drive_sub_path = os.path.join(DRIVE_CKPT_DIR, 'submission_phase12a.csv')
...
print("submission_phase12a.csv done", flush=True)
print(submission.describe(), flush=True)

# ============================================================
# 11. Feature Importance (LGB raw+MAE)
# ============================================================
...
if importance is not None:
    ...
    # Count q_ features in top 30
    q_in_top30 = top30['feature'].str.startswith('q_').sum()
    print(f"  Queueing theory features in Top 30: {q_in_top30}", flush=True)

    # Color: q_ features highlighted
    colors = ['orangered' if f.startswith('q_') else 'steelblue' for f in top30['feature']]

    fig, ax = plt.subplots(figsize=(10, 10))
    ax.barh(top30['feature'], top30['importance'], color=colors)
    ax.set_title('Feature Importance Top 30 (Phase 12a - LGB raw+MAE)')
    ax.set_xlabel('Importance')
    legend_elements = [Patch(facecolor='steelblue', label='Existing'),
                       Patch(facecolor='orangered', label='Queueing Theory (q_)')]
    ax.legend(handles=legend_elements, loc='lower right')
    plt.tight_layout()
    plt.savefig('output/feature_importance_phase12a.png', dpi=150, bbox_inches='tight')
    print("feature_importance_phase12a.png saved", flush=True)

    # Print all q_ features importance
