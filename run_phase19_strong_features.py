...
PIPELINE_VERSION = "phase19_strong_features_v1"
...
ckpt_path = 'output/ckpt_phase19_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase19_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
try:
    ...
    ckpt_path = 'output/ckpt_phase19_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
print(f"\n=== Phase 19 Ensemble CV ===", flush=True)
...
print(f"Improvement vs P17: {8.4251 - ensemble_cv:+.4f}", flush=True)
...
sub.to_csv('output/submission_phase19_strong_features.csv', index=False)
shutil.copy('output/submission_phase19_strong_features.csv',
            os.path.join(DRIVE_CKPT_DIR, 'submission_phase19_strong_features.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"\nsubmission_phase19_strong_features.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 19 Results ===", flush=True)
print("=" * 60, flush=True)
print(f"Base (Phase 16): {len(phase16_feature_cols)}", flush=True)
print(f"Phase 17 new:    {len(selected_new)}", flush=True)
print(f"Phase 2B new:    {len(new_2b)} (of {len(PHASE2B_FEATURES)})", flush=True)
...
print(f"Phase 19:   {ensemble_cv:.4f}", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    n_p17 = top30['feature'].isin(selected_new).sum()
    n_p2b = top30['feature'].isin(new_2b).sum()
    print(f"\n  Phase 17 features in Top 30: {n_p17}", flush=True)
    print(f"  Phase 2B features in Top 30: {n_p2b}", flush=True)
    colors = ['purple' if f in new_2b else ('darkgreen' if f in selected_new else ('crimson' if f in selected_2nd else ('orangered' if f.startswith(('s_', 'l_', 'h_', 'lh_', 'r_')) else 'steelblue'))) for f in top30['feature']]
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.barh(top30['feature'], top30['importance'], color=colors)
    ax.set_title('Feature Importance Top 30 (Phase 19)')
    ax.legend(handles=[Patch(facecolor='steelblue', label='Base'), Patch(facecolor='orangered', label='P15 Agg'), Patch(facecolor='crimson', label='P16 2nd-order'), Patch(facecolor='darkgreen', label='P17 New'), Patch(facecolor='purple', label='P19 Phase2B')], loc='lower right')
    plt.tight_layout(); plt.savefig('output/feature_importance_phase19.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase19.png saved", flush=True)

print("\n=== Phase 19 Complete ===", flush=True)
