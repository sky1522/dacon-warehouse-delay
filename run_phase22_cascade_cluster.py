...
new_imp2.to_csv('output/phase22_feature_importance.csv', index=False)
del X_tmp2, sel2; gc.collect()

# ##############################################################
# Part 3B: Phase 22 - Cascading + Layout Cluster Features
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Part 3B: Cascading Binary + Layout Cluster ===", flush=True)
print("=" * 60, flush=True)

# A. Cascading Binary (5 features)
print("\n=== A) Cascading Binary ===", flush=True)
combined['rho_robot'] = (combined['robot_active'] / (combined['robot_total'] + 1e-6)).astype('float32')
combined['rho_pack'] = combined['pack_utilization'].fillna(0.3).astype('float32')
combined['rho_charger'] = (combined['charge_queue_length'] / (combined['charger_count'] + 1e-6)).astype('float32')
combined['rho_max_new'] = combined[['rho_robot', 'rho_pack', 'rho_charger']].max(axis=1).astype('float32')

combined['rho_over_70'] = (combined['rho_max_new'] > 0.70).astype('float32')
combined['rho_over_85'] = (combined['rho_max_new'] > 0.85).astype('float32')
combined['rho_over_95'] = (combined['rho_max_new'] > 0.95).astype('float32')

combined['robot_pressure'] = (combined['rho_robot'] > 0.85).astype('float32')
combined['pack_pressure'] = (combined['pack_utilization'] > 0.80).astype('float32')
combined['charger_pressure'] = (combined['charge_queue_length'] > 5).astype('float32')
combined['multi_pressure'] = (combined['robot_pressure'] + combined['pack_pressure'] + combined['charger_pressure']).astype('float32')

combined['explosion_intensity'] = (combined['rho_max_new'] * combined['multi_pressure']).astype('float32')
print("  5 cascading features done", flush=True)

# B. Layout Cluster (4 features)
print("=== B) Layout Cluster ===", flush=True)
layout_info = pd.read_csv('data/layout_info.csv')
layout_cluster_features = ['aisle_width_avg', 'robot_total', 'intersection_count',
                           'layout_compactness', 'pack_station_count', 'charger_count',
                           'one_way_ratio', 'floor_area_sqm', 'zone_dispersion']
X_lc = layout_info[layout_cluster_features].fillna(0).values
X_lc_scaled = StandardScaler().fit_transform(X_lc)
km = KMeans(n_clusters=10, random_state=42, n_init=10)
layout_info['layout_cluster'] = km.fit_predict(X_lc_scaled)

# Cluster별 통계 (train만 사용, test leakage 방지)
train_df_tmp = combined[combined['_is_train'] == 1].merge(
    layout_info[['layout_id', 'layout_cluster']], on='layout_id'
)
cluster_stats = train_df_tmp.groupby('layout_cluster').agg(
...
# combined에 merge
combined = combined.merge(layout_info[['layout_id', 'layout_cluster']], on='layout_id', how='left', suffixes=('', '_dup'))
if 'layout_cluster_dup' in combined.columns:
    combined.drop(columns=['layout_cluster_dup'], inplace=True)
combined = combined.merge(cluster_stats, on='layout_cluster', how='left')
for col in ['cluster_mean_target', 'cluster_p95_target', 'cluster_bin9_rate', 'cluster_size']:
    combined[col] = combined[col].fillna(0).astype('float32')
print(f"  4 cluster features done (10 clusters)", flush=True)

# C. Cross Features (3 features)
print("=== C) Cross Features ===", flush=True)
# layout_compactness, aisle_width_avg are in layout_info, need to bring them back
li_cross = layout_info[['layout_id', 'layout_compactness', 'aisle_width_avg']].copy()
combined = combined.merge(li_cross, on='layout_id', how='left', suffixes=('', '_li'))
lc_col = 'layout_compactness_li' if 'layout_compactness_li' in combined.columns else 'layout_compactness'
aw_col = 'aisle_width_avg_li' if 'aisle_width_avg_li' in combined.columns else 'aisle_width_avg'

combined['rho85_x_clusterbin9'] = (combined['rho_over_85'] * combined['cluster_bin9_rate']).astype('float32')
combined['multipress_x_compact'] = (combined['multi_pressure'] * combined[lc_col].fillna(0)).astype('float32')
combined['explosion_x_narrow'] = (combined['explosion_intensity'] * (combined[aw_col].fillna(3.0) < 2.5).astype(int)).astype('float32')

# Cleanup temp columns
for c in [lc_col, aw_col]:
    if c.endswith('_li') and c in combined.columns:
        combined.drop(columns=[c], inplace=True)
print("  3 cross features done", flush=True)

# Phase 22 new feature list
phase22_new_feats = [
    'rho_over_70', 'rho_over_85', 'rho_over_95', 'multi_pressure', 'explosion_intensity',
    'cluster_mean_target', 'cluster_p95_target', 'cluster_bin9_rate', 'cluster_size',
    'rho85_x_clusterbin9', 'multipress_x_compact', 'explosion_x_narrow',
]
# Drop intermediate columns not needed as features
for c in ['rho_robot', 'rho_pack', 'rho_charger', 'rho_max_new', 'robot_pressure', 'pack_pressure', 'charger_pressure', 'layout_cluster']:
    if c in combined.columns:
        combined.drop(columns=[c], inplace=True)

final_feature_cols = phase16_feature_cols + [f for f in phase22_new_feats if f in combined.columns]
print(f"\n  Phase 22 total features: {len(final_feature_cols)} (+{len(final_feature_cols) - len(phase16_feature_cols)} new)", flush=True)
...
ckpt_path = 'output/ckpt_phase22_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase22_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
print(f"\n  Phase 22 Ensemble CV: {ensemble_cv:.4f}", flush=True)
...
sub.to_csv('output/submission_phase22.csv', index=False)
shutil.copy('output/submission_phase22.csv', os.path.join(DRIVE_CKPT_DIR, 'submission_phase22.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"  submission_phase22.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 22 Results (Cascading + Layout Cluster) ===", flush=True)
print("=" * 60, flush=True)
print(f"Phase 16 base:       {len(phase16_feature_cols)}", flush=True)
print(f"Phase 22 new:        {len(phase22_new_feats)}", flush=True)
print(f"Total:               {len(final_feature_cols)}", flush=True)
for n in model_names:
    print(f"  {n:12s}: {cv_maes[n]:.4f}", flush=True)
print(f"\nPhase 22 Ensemble CV: {ensemble_cv:.4f}", flush=True)
print(f"Phase 16 baseline:    8.4403", flush=True)
print(f"Delta:                {8.4403 - ensemble_cv:+.4f}", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    n_p22 = top30['feature'].isin(phase22_new_feats).sum()
    n_2nd = top30['feature'].isin(selected_2nd).sum()
    print(f"\n  Phase 22 new in Top 30: {n_p22}", flush=True)
    print(f"  2nd-order in Top 30:   {n_2nd}", flush=True)
    # Show Phase 22 new feature importance ranks
    p22_imp = imp[imp['feature'].isin(phase22_new_feats)]
    print(f"\n  Phase 22 new feature importance:", flush=True)
    for rank, (_, row) in enumerate(p22_imp.iterrows(), 1):
        overall_rank = (imp['feature'] == row['feature']).values.argmax() + 1
        print(f"    {row['feature']:30s}: imp={row['importance']:6.0f} (rank {overall_rank}/{len(feature_cols)})", flush=True)
    colors = ['green' if f in phase22_new_feats else ('crimson' if f in selected_2nd else ('orangered' if f.startswith(('s_', 'l_', 'h_', 'lh_', 'r_')) else 'steelblue')) for f in top30['feature']]
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.barh(top30['feature'], top30['importance'], color=colors)
    ax.set_title('Feature Importance Top 30 (Phase 22)')
    ax.legend(handles=[Patch(facecolor='steelblue', label='Base'), Patch(facecolor='orangered', label='P15 Agg'), Patch(facecolor='crimson', label='2nd-order'), Patch(facecolor='green', label='P22 New')], loc='lower right')
    plt.tight_layout(); plt.savefig('output/feature_importance_phase22.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase22.png saved", flush=True)
