...
new_imp2.to_csv('output/phase21b_feature_importance.csv', index=False)
...
def build_sample_weight(y_arr, time_arr):
    ...
    return w
base_w = build_sample_weight(y.values, time_idx)

# ==== Phase 21B: Adversarial weight ====
print("\n=== Phase 21B: Adversarial Validation ===", flush=True)
...
# Adversarial weight: clip [0.5, 2.0] (conservative)
adv_weight = np.clip(train_adv_proba / (1 - train_adv_proba + 1e-6), 0.5, 2.0).astype('float32')
...
# Combine: base_weight * adv_weight
sample_w = base_w * adv_weight
print(f"  Final sample_w: mean={sample_w.mean():.3f}, std={sample_w.std():.3f}", flush=True)
...
ckpt_path = 'output/ckpt_phase21b_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21b_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
print(f"\n  Phase 21B Ensemble CV: {ensemble_cv:.4f}", flush=True)
...
sub.to_csv('output/submission_phase21b.csv', index=False)
shutil.copy('output/submission_phase21b.csv', os.path.join(DRIVE_CKPT_DIR, 'submission_phase21b.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"  submission_phase21b.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 21B Results (adversarial weight isolated test) ===", flush=True)
print("=" * 60, flush=True)
print(f"Change: adversarial weight (GroupKFold, clip [0.5, 2.0]), AUC={adv_auc:.4f}", flush=True)
...
if ensemble_cv < 8.4403:
    print(f"  → Adversarial weight is EFFECTIVE", flush=True)
else:
    print(f"  → Adversarial weight has NO EFFECT", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    ax.set_title('Feature Importance Top 30 (Phase 21B)')
    ...
    plt.tight_layout(); plt.savefig('output/feature_importance_phase21b.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase21b.png saved", flush=True)

print("\n=== Phase 21B Complete ===", flush=True)
