...
# ==== Phase 21A Change: fillna(median) for 33 key columns ====
print("=== Phase 21A: Median Fill (33 key columns) ===", flush=True)
...
new_imp2.to_csv('output/phase21a_feature_importance.csv', index=False)
...
ckpt_path = 'output/ckpt_phase21a_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase21a_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
print(f"\n  Phase 21A Ensemble CV: {ensemble_cv:.4f}", flush=True)
...
sub.to_csv('output/submission_phase21a.csv', index=False)
shutil.copy('output/submission_phase21a.csv', os.path.join(DRIVE_CKPT_DIR, 'submission_phase21a.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"  submission_phase21a.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 21A Results (fillna(median) isolated test) ===", flush=True)
print("=" * 60, flush=True)
print(f"Change: fillna(median) for 33 key columns (train median)", flush=True)
print(f"Base (Phase 15): {len(feature_cols_p15)}", flush=True)
print(f"New 2nd-order:   {len(selected_2nd)}", flush=True)
...
if ensemble_cv < 8.4403:
    print(f"  → fillna(median) is EFFECTIVE", flush=True)
else:
    print(f"  → fillna(median) has NO EFFECT (tree models handle NaN natively)", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    ax.set_title('Feature Importance Top 30 (Phase 21A)')
    ...
    plt.tight_layout(); plt.savefig('output/feature_importance_phase21a.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase21a.png saved", flush=True)

print("\n=== Phase 21A Complete ===", flush=True)
