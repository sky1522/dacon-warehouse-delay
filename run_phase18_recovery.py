...
PIPELINE_VERSION = "phase18_recovery_v1"
...
print(f"\n  Phase 16 features: {len(phase16_feature_cols)}", flush=True)
new_imp2.to_csv('output/phase18r_feature_importance.csv', index=False)
del X_tmp2, sel2; gc.collect()

# ##############################################################
# Part 3B: Composite Features (H3 - 3개만 추가)
# ##############################################################
print("\n=== Part 3B: Composite Features (H3) ===", flush=True)
...
combined = add_composite_features(combined)
composite_feats = ['service_impediment', 'crisis_indicator', 'paradox_indicator']
final_feature_cols = phase16_feature_cols + composite_feats
print(f"  Added 3 composite features", flush=True)
...
# --- Model 1: LGB log1p+MAE (H1: was raw+MAE) ---
print("\n  [1] LGB log1p+MAE", flush=True)
ckpt_path = 'output/ckpt_phase18r_lgb_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
if ckpt: oof_preds['lgb_raw'] = ckpt['oof']; test_preds['lgb_raw'] = ckpt['test']; cv_maes['lgb_raw'] = ckpt['cv_mae']; m1_models = []
    ...
    for fi, (tri, vai) in enumerate(folds):
        ...
        p = np.clip(np.expm1(m.predict(X.iloc[vai])), 0, None).astype(np.float32); o[vai] = p; t += np.clip(np.expm1(m.predict(X_test)), 0, None).astype(np.float32) / 5; m1_models.append(m)
...
ckpt_path = 'output/ckpt_phase18r_lgb_huber.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
# --- Model 3: LGB log1p+MAE-v2 (H1: was sqrt) ---
print("\n  [3] LGB log1p+MAE-v2", flush=True)
ckpt_path = 'output/ckpt_phase18r_lgb_sqrt.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
# --- Model 4: XGB log1p (H1: was raw) ---
print("\n  [4] XGB log1p", flush=True)
ckpt_path = 'output/ckpt_phase18r_xgb.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
if ckpt: oof_preds['xgb'] = ckpt['oof']; test_preds['xgb'] = ckpt['test']; cv_maes['xgb'] = ckpt['cv_mae']
    ...
    for fi, (tri, vai) in enumerate(folds):
        ...
        m.fit(X.iloc[tri], y_log.iloc[tri], sample_weight=sample_w[tri], eval_set=[(X.iloc[vai], y_log.iloc[vai])], verbose=False)
        p = np.clip(np.expm1(m.predict(X.iloc[vai])), 0, None).astype(np.float32); o[vai] = p; t += np.clip(np.expm1(m.predict(X_test)), 0, None).astype(np.float32) / 5
        print(f"    F{fi+1} MAE: {mean_absolute_error(y.iloc[vai], p):.4f}", flush=True)
...
ckpt_path = 'output/ckpt_phase18r_cat_log1p.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
# --- Model 6: Cat log1p (H1: was raw) ---
print("\n  [6] Cat log1p", flush=True)
ckpt_path = 'output/ckpt_phase18r_cat_raw.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase18r_mlp.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
...
ckpt_path = 'output/ckpt_phase18r_tabnet.pkl'; ckpt = load_ckpt(ckpt_path, expected_features=feature_cols)
if ckpt: oof_preds['tabnet'] = ckpt['oof']; test_preds['tabnet'] = ckpt['test']; cv_maes['tabnet'] = ckpt['cv_mae']
    ...
    for fi, (tri, vai) in enumerate(folds):
        ...
        mdl.fit(X_train=Xtr, y_train=y_log.values[tri].reshape(-1, 1).astype('float32'), eval_set=[(Xva, y_log.values[vai].reshape(-1, 1).astype('float32'))], eval_metric=['mae'], max_epochs=50, patience=15, batch_size=2048, virtual_batch_size=256, num_workers=0, drop_last=False)
        p = np.clip(np.expm1(mdl.predict(Xva).flatten()), 0, None).astype(np.float32); o[vai] = p
        t += np.clip(np.expm1(mdl.predict(Xte).flatten()), 0, None).astype(np.float32) / 5
        print(f"    F{fi+1} MAE: {mean_absolute_error(y.values[vai], p):.4f}", flush=True)
        del mdl; gc.collect()
...
print(f"\n  Phase 16 Ensemble CV: {ensemble_cv:.4f}", flush=True)
print(f"  Phase 15 baseline:   8.4553", flush=True)
print(f"  Improvement:         {8.4553 - ensemble_cv:+.4f}", flush=True)
print(f"\n  Weights:", flush=True)
...
sub.to_csv('output/submission_phase18_recovery.csv', index=False)
shutil.copy('output/submission_phase18_recovery.csv', os.path.join(DRIVE_CKPT_DIR, 'submission_phase18_recovery.csv'))
assert (sub['ID'] == sample_sub['ID']).all()
assert (sub['avg_delay_minutes_next_30m'] >= 0).all()
print(f"  submission_phase18_recovery.csv saved", flush=True)
...
# ##############################################################
print("\n" + "=" * 60, flush=True)
print("=== Phase 18 Recovery Results (H1: log1p + H3: composite) ===", flush=True)
print("=" * 60, flush=True)
print(f"Phase 16 base:   {len(phase16_feature_cols)}", flush=True)
print(f"Composite (H3):  3", flush=True)
print(f"Total:           {len(final_feature_cols)}", flush=True)
print(f"\nH1 change: ALL models train on log1p target", flush=True)
for n in model_names:
    print(f"  {n:12s}: {cv_maes[n]:.4f}", flush=True)
print(f"\nPhase 18R Ensemble CV: {ensemble_cv:.4f}", flush=True)
print(f"Phase 16 baseline:     8.3959", flush=True)
print(f"Delta:                 {ensemble_cv - 8.3959:+.4f}", flush=True)
...
if 'm1_models' in dir() and len(m1_models) > 0:
    ...
    ax.set_title('Feature Importance Top 30 (Phase 18 Recovery)')
    ...
    plt.tight_layout(); plt.savefig('output/feature_importance_phase18r.png', dpi=150, bbox_inches='tight')
    print("  feature_importance_phase18r.png saved", flush=True)

print("\n=== Phase 18 Recovery Complete ===", flush=True)
