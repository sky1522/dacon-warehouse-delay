...
def load_ckpt(local_path):
    ...
    if os.path.exists(drive_path):
        with open(drive_path, 'rb') as f:
            print(f"  ⏭️ Drive 캐시 로드: {drive_path}", flush=True)
            return pickle.load(f)
    if os.path.exists(local_path):
        with open(local_path, 'rb') as f:
            print(f"  ⏭️ 로컬 캐시 로드: {local_path}", flush=True)
            return pickle.load(f)
    return None
...
# sample weight 생성: 완화된 adversarial weight (max ~1.8)
adv_weight = 1.0 + 0.3 * train_test_score  # 1.0 ~ ~1.3 (완화)
...
ckpt_path = 'output/ckpt_phase11b_lgb_raw.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m1_oof, m1_test, cv_maes['lgb_raw_mae'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    m1_models = []
    print(f"  CV MAE: {cv_maes['lgb_raw_mae']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ lgb_raw 완료 (CV MAE: {cv_maes['lgb_raw_mae']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_lgb_huber.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m2_oof, m2_test, cv_maes['lgb_log1p_huber'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {cv_maes['lgb_log1p_huber']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ lgb_huber 완료 (CV MAE: {cv_maes['lgb_log1p_huber']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_lgb_sqrt.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m3_oof, m3_test, cv_maes['lgb_sqrt_mae'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {cv_maes['lgb_sqrt_mae']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ lgb_sqrt 완료 (CV MAE: {cv_maes['lgb_sqrt_mae']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_xgb.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m4_oof, m4_test, cv_maes['xgb_raw_mae'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {cv_maes['xgb_raw_mae']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ xgb 완료 (CV MAE: {cv_maes['xgb_raw_mae']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_cat_log1p.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m5_oof, m5_test, cv_maes['cat_log1p_mae'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {cv_maes['cat_log1p_mae']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ cat_log1p 완료 (CV MAE: {cv_maes['cat_log1p_mae']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_cat_raw.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    m6_oof, m6_test, cv_maes['cat_raw_mae'] = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {cv_maes['cat_raw_mae']:.4f}", flush=True)
else:
    ...
    print(f"  ✅ cat_raw 완료 (CV MAE: {cv_maes['cat_raw_mae']:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_mlp.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    mlp_oof, mlp_test, mlp_cv = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {mlp_cv:.4f}", flush=True)
else:
    ...
    print(f"  ✅ mlp 완료 (CV MAE: {mlp_cv:.4f})", flush=True)
...
ckpt_path = 'output/ckpt_phase11b_tabnet.pkl'
ckpt = load_ckpt(ckpt_path)
if ckpt is not None:
    tabnet_oof, tabnet_test, tabnet_cv = ckpt['oof'], ckpt['test'], ckpt['cv_mae']
    print(f"  CV MAE: {tabnet_cv:.4f}", flush=True)
else:
    ...
    print(f"  ✅ tabnet 완료 (CV MAE: {tabnet_cv:.4f})", flush=True)
...
# ============================================================
# 7. Level 1 가중 평균 앙상블 (8모델)
# ============================================================
print("\n=== Level 1 가중 평균 앙상블 (8모델) ===", flush=True)
...
print("\n  트리+NN 가중치 (8모델):", flush=True)
for n, w in zip(model_names, all_w):
    print(f"    {n:20s}: {w:.4f}", flush=True)
print(f"  트리+NN Level 1 CV MAE (8모델): {all_cv:.4f}", flush=True)

# ============================================================
# 8. Level 2 LGB 스태킹 (8모델)
# ============================================================
...
print(f"  메타 피처 수: {len(meta_feature_names)} (8 OOF + 6 원본)", flush=True)
...
candidates = {
    ...
    'all_l1_8model': (all_cv, oof_matrix @ all_w, test_matrix @ all_w),
    'lgb_stack_8model': (meta_lgb_cv, meta_lgb_oof, meta_lgb_test),
}
...
# ============================================================
print("\n" + "=" * 60, flush=True)
print("=== Phase 11b 결과 ===", flush=True)
print("=" * 60, flush=True)
print(f"Adversarial AUC: {auc:.4f}", flush=True)
print(f"Adv weight (완화): 1.0 + 0.3 * score (max ~1.3)", flush=True)
...
print(f"Level 1 가중 평균 (8모델):     {all_cv:.4f}", flush=True)
...
# 로컬 저장
submission.to_csv('output/submission_phase11b.csv', index=False)
# Drive 저장
drive_sub_path = os.path.join(DRIVE_CKPT_DIR, 'submission_phase11b.csv')
...
print(f"submission_phase11b.csv 생성 완료 (로컬 + Drive)", flush=True)
print(submission.describe(), flush=True)

# ============================================================
# 11. 피처 중요도 (LGB raw+MAE)
# ============================================================
...
if len(m1_models) > 0:
    ...
    ax.set_title('Feature Importance Top 30 (Phase 11b - LGB raw+MAE + Soft Adv Weight)')
    ax.set_xlabel('Importance')
    plt.tight_layout()
    plt.savefig('output/feature_importance_phase11b.png', dpi=150, bbox_inches='tight')
    print("feature_importance_phase11b.png 저장 완료", flush=True)
else:
    print("  (캐시 사용으로 피처 중요도 시각화 건너뜀)", flush=True)

print("\n=== Phase 11b 완료 ===", flush=True)
